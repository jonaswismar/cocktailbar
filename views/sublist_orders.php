<?php
	...
	while($orders_all_rows= mysqli_fetch_array($orders_all_res, MYSQLI_ASSOC)){
		...
		$username_stmt = mysqli_prepare($link, $sql_users_singlename);
		mysqli_stmt_bind_param($username_stmt, "i", $orders_all_rows['user']);
		mysqli_stmt_execute($username_stmt);
		$username_all_res=mysqli_stmt_get_result($username_stmt);
		while($username_all_rows= mysqli_fetch_array($username_all_res, MYSQLI_ASSOC)){
...
<?php
	...
	if(file_exists($currentfilepath .'/img/cocktails/webp128/' . $orders_all_rows['image'] . '.webp')){
		echo '						<source srcset="../img/cocktails/webp128/' . $orders_all_rows['image'] . '.webp" type="image/webp" media="(min-width: 1600px)">
';
	}
	if(file_exists($currentfilepath .'/img/cocktails/webp64/' . $orders_all_rows['image'] . '.webp')){
		echo '						<source srcset="../img/cocktails/webp64/' . $orders_all_rows['image'] . '.webp" type="image/webp" media="(min-width: 960px)">
';
	}
	if(file_exists($currentfilepath .'/img/cocktails/webp48/' . $orders_all_rows['image'] . '.webp')){
		echo '						<source srcset="../img/cocktails/webp48/' . $orders_all_rows['image'] . '.webp" type="image/webp">
';
	}
	if(file_exists($currentfilepath .'/img/cocktails/png128/' . $orders_all_rows['image'] . '.png')){
		echo '						<source srcset="../img/cocktails/png128/' . $orders_all_rows['image'] . '.png" type="image/png" media="(min-width: 1600px)">
';
	}
	if(file_exists($currentfilepath .'/img/cocktails/png64/' . $orders_all_rows['image'] . '.png')){
		echo '						<source srcset="../img/cocktails/png64/' . $orders_all_rows['image'] . '.png" type="image/png" media="(min-width: 960px)">
';
	}
	if(file_exists($currentfilepath .'/img/cocktails/png48/' . $orders_all_rows['image'] . '.png')){
		echo '						<source srcset="../img/cocktails/png48/' . $orders_all_rows['image'] . '.png" type="image/png">
';
	}
	if(file_exists($currentfilepath .'/img/glassware/webp128/' . $orders_all_rows['glass'] . '.webp')){
		echo '						<source srcset="../img/glassware/webp128/' . $orders_all_rows['glass'] . '.webp" type="image/webp" media="(min-width: 1600px)">
';
	}
	if(file_exists($currentfilepath .'/img/glassware/webp64/' . $orders_all_rows['glass'] . '.webp')){
		echo '						<source srcset="../img/glassware/webp64/' . $orders_all_rows['glass'] . '.webp" type="image/webp" media="(min-width: 960px)">
';
	}
	if(file_exists($currentfilepath .'/img/glassware/webp48/' . $orders_all_rows['glass'] . '.webp')){
		echo '						<source srcset="../img/glassware/webp48/' . $orders_all_rows['glass'] . '.webp" type="image/webp">
';
	}
	if(file_exists($currentfilepath .'/img/glassware/jpg128/' . $orders_all_rows['glass'] . '.jpg')){
		echo '						<source srcset="../img/glassware/jpg128/' . $orders_all_rows['glass'] . '.jpg" type="image/jpg" media="(min-width: 1600px)">
';
	}
	if(file_exists($currentfilepath .'/img/glassware/jpg64/' . $orders_all_rows['glass'] . '.jpg')){
		echo '						<source srcset="../img/glassware/jpg64/' . $orders_all_rows['glass'] . '.jpg" type="image/jpg" media="(min-width: 960px)">
';
	}
	if(file_exists($currentfilepath .'/img/glassware/jpg48/' . $orders_all_rows['glass'] . '.jp')){
		echo '						<source srcset="../img/glassware/jpg48/' . $orders_all_rows['glass'] . '.jpg" type="image/jpg">
';
	}
?>
					...
					<div class="flex-grow-1 d-flex gap-2 w-75 flex-nowrap">
						<div class="d-flex-column">
							...
							<p class="mb-0 text-size-h5 opacity-75 text-truncate textcalcoffset">am <strong><?php echo date_format($orderdate,"d.m.Y"); ?></strong> um <strong><?php echo date_format($orderdate,"H:i"); ?></strong> von <strong><?php echo $username; ?></strong> in <strong><?php echo $orders_all_rows['barname']; ?></strong></p>
						</div>
					</div>
